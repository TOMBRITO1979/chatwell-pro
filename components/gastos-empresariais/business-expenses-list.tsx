...
import { Briefcase, Plus, Search, Edit, Trash2, DollarSign, Filter, Calendar } from 'lucide-react';
...
export function BusinessExpensesList() {
  ...
  const loadExpenses = async () => {
    try {
      ...
      console.error('Erro ao carregar gastos empresariais:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/business-expenses/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleToggleStatus = async (expense: BusinessExpense) => {
    try {
      const token = localStorage.getItem('token');
  ...
  const categories = ['salarios', 'aluguel', 'utilities', 'marketing', 'equipamentos', 'viagens', 'fornecedores', 'impostos', 'servicos', 'outros'];
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Controle e organize as despesas da empresa
          </p>
        </div>
        <Button
